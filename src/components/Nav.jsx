...
export default function Nav() {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-100 flex items-center justify-between py-[1.6rem] px-[4rem] max-sm:px-[1.25rem] sm:max-lg:px-[2rem] transition-all duration-[400ms] ${scrolled ? 'bg-white/92 backdrop-blur-[16px] border-b border-line' : 'bg-transparent'}`}>
      ...
      <ul className="flex max-sm:gap-4 sm:max-md:gap-5 gap-9">
        {[
          ["Work", "#work"],
          ["About", "#info"],
          ["Contact", "#info"],
          ...
          <li key={label}>
            <a
              href={href}
              className="max-sm:text-[0.75rem] sm:max-md:text-[0.8rem] text-[0.95rem] font-semibold tracking-[0.1em] uppercase text-black hover:text-blue transition-colors duration-300"
            >
              {label}
            </a>
          </li>
